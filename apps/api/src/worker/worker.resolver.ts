...
export class WorkerResolver {
  ...
  @UseGuards(AuthGuard, RolesGuard)
  @RequireRoles('WORKER')
  async syncProfessionalServices(
    @Args('input', { type: () => SyncProfessionalServicesInput }) input: SyncProfessionalServicesInput,
    @CurrentUser() user: any
  ): Promise<Service[]> {
    // Get worker profile
    const worker = await (this.prisma as any).workerProfile.findUnique({
    ...
    return this.workerService.syncProfessionalServices(worker.id, input.services);
  }
}
